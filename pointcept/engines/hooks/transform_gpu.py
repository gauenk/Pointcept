    ...
    def __call__(self, data_dict, _idx=None):
        ...
        if True:
            ...
            if "sampled_index" in data_dict:
                ...
                # update sampled_index relative to kept indices
                mask = torch.zeros(n, dtype=torch.bool, device="cuda")
                mask[sampled_idx] = True
                data_dict["sampled_index"] = torch.nonzero(mask[idx], as_tuple=False).squeeze(1)
...
class RandomRotate(object):
    ...
    def version_v0(self,data_dict): # for benchmarking 
        ...
        if self.axis == "x":
            rot_t = torch.tensor([[1, 0, 0],
                                  [0, rot_cos, -rot_sin],
                                  [0, rot_sin, rot_cos]], device="cuda", dtype=torch.float32)
        elif self.axis == "y":
            rot_t = torch.tensor([[rot_cos, 0, rot_sin],
                                  [0, 1, 0],
                                  [-rot_sin, 0, rot_cos]], device="cuda", dtype=torch.float32)
        elif self.axis == "z":
            rot_t = torch.tensor([[rot_cos, -rot_sin, 0],
                                  [rot_sin, rot_cos, 0],
        ...
        if "coord" in data_dict:
            ...
            coords = torch.matmul(coords, rot_t.T)
            coords = coords + center
            data_dict["coord"] = coords
...
class ClipGaussianJitter(object):
    def __init__(self, scalar=0.02, store_jitter=False):
        self.scalar = scalar
        self.mean = torch.zeros(3, device="cuda")  # zero-mean
        self.cov = torch.eye(3, device="cuda")     # identity covariance
        self.quantile = 1.96
...
class ChromaticAutoContrast(object):
    ...
    def __call__(self, data_dict):
        ...
        for b in range(B):
            ...
            diff = hi - lo
            scale = torch.where(diff < 1e-10, torch.ones_like(diff), 255.0 / diff)
...
class RandomColorGrayScale(object):
    ...
    def rgb_to_grayscale(color, num_output_channels=1):
        if color.shape[-1] < 3:
            raise TypeError(f"Input color should have at least 3 channels, but found {color.shape[-1]}")
        ...
        if num_output_channels == 3:
            gray = gray.expand_as(color)  # broadcast to 3 channels
...
class RandomColorJitter(object):
    """
    Random Color Jitter for 3D point cloud (GPU-accelerated version)
    """
    ...
    @staticmethod
    def rgb2hsv(rgb):
        """Convert RGB to HSV color space on GPU"""
        r, g, b = rgb[..., 0], rgb[..., 1], rgb[..., 2]
        maxc = torch.max(rgb, dim=-1)[0]
        minc = torch.min(rgb, dim=-1)[0]
        eqc = (maxc == minc).float()
        cr = maxc - minc
        s = cr / (torch.ones_like(maxc) * eqc + maxc * (1 - eqc))
        cr_divisor = torch.ones_like(maxc) * eqc + cr * (1 - eqc)
    ...
    @staticmethod
    def hsv2rgb(hsv):
        """Convert HSV to RGB color space on GPU"""
        ...
        mask = (i.unsqueeze(-1) == torch.arange(6, device=hsv.device)).float()
        ...
        return torch.einsum("...na,...nab->...nb", mask.to(hsv.dtype), a4)
    ...
    def adjust_brightness(self, color, brightness_factor):
        ...
        return self.blend(color, torch.zeros_like(color), brightness_factor)
    ...
    def adjust_contrast(self, color, contrast_factor):
        ...
        # Assuming rgb_to_grayscale is available from RandomColorGrayScale
    ...
    def adjust_saturation(self, color, saturation_factor):
        ...
        gray_weights = torch.tensor([0.299, 0.587, 0.114], 
                                    device=color.device, 
                                    dtype=color.dtype)
        gray = torch.sum(color * gray_weights, dim=-1, keepdim=True)
    ...
    def adjust_hue(self, color, hue_factor):
        ...
        color_hue_adj = (self.hsv2rgb(hsv) * 255.0).to(orig_dtype)
        return color_hue_adj

    @staticmethod
    def get_params(brightness, contrast, saturation, hue):
        """Generate random parameters for color jittering"""
        fn_idx = torch.randperm(4)
        b = (
            None
            if brightness is None
            else torch.empty(1).uniform_(brightness[0], brightness[1]).item()
        )
        c = (
            None
            if contrast is None
            else torch.empty(1).uniform_(contrast[0], contrast[1]).item()
        )
        s = (
            None
            if saturation is None
            else torch.empty(1).uniform_(saturation[0], saturation[1]).item()
        )
        h = (
            None
            if hue is None
            else torch.empty(1).uniform_(hue[0], hue[1]).item()
        )
        return fn_idx, b, c, s, h

    def __call__(self, data_dict):
        """Apply random color jittering to point cloud colors"""
        (
            fn_idx,
            brightness_factor,
            contrast_factor,
        ...
        for fn_id in fn_idx:
            if (
                ...
                fn_id == 1 
                and contrast_factor is not None 
                and torch.rand(1).item() < self.p
            ):
...
class HueSaturationTranslation(object):
    """GPU-accelerated Hue and Saturation transformation for point clouds"""

    @staticmethod
    def rgb_to_hsv(rgb):
        """
        Convert RGB to HSV color space on GPU
        rgb: tensor with values between 0 and 255
        returns: tensor with h,s in [0,1] and v in [0,255]
        """
        rgb = rgb.float()
    ...
    def hsv_to_rgb(hsv):
        """
        Convert HSV to RGB color space on GPU
        hsv: tensor with h,s in [0,1] and v in [0,255]
        returns: uint8 tensor with values between 0 and 255
        """
        ...
        rgb[..., 0] = torch.where(
            cond_s_zero, v,
            torch.where(cond_i_1, q,
            torch.where(cond_i_2, p,
            torch.where(cond_i_3, p,
            torch.where(cond_i_4, t,
            torch.where(cond_i_5, v, v)))))
        )
        
        # G channel
        rgb[..., 1] = torch.where(
            cond_s_zero, v,
            torch.where(cond_i_1, v,
            torch.where(cond_i_2, v,
            torch.where(cond_i_3, q,
            torch.where(cond_i_4, p,
            torch.where(cond_i_5, p, t)))))
        )
        
        # B channel
        rgb[..., 2] = torch.where(
            cond_s_zero, v,
            torch.where(cond_i_1, p,
            torch.where(cond_i_2, t,
            torch.where(cond_i_3, v,
            torch.where(cond_i_4, v,
            torch.where(cond_i_5, q, p)))))
        )
    ...
    def __call__(self, data_dict):
        if "color" in data_dict.keys():
            ...
            # Generate random values on same device as input
            device = data_dict["color"].device
            hue_val = (torch.rand(1, device=device).item() - 0.5) * 2 * self.hue_max
            sat_ratio = 1 + (torch.rand(1, device=device).item() - 0.5) * 2 * self.saturation_max
...
class RandomColorDrop(object):
    """GPU-accelerated Random Color Drop for point clouds"""
    ...
    def __call__(self, data_dict):
        if "color" in data_dict.keys() and torch.rand(1).item() < self.p:
...
class ElasticDistortion(object):
    """GPU-accelerated Elastic Distortion for point clouds"""
    ...
    def elastic_distortion(coords, granularity, magnitude):
        """
        Apply elastic distortion on sparse coordinate space using GPU.
        coords: torch tensor of (number of points, at least 3 spatial dims)
        granularity: size of the noise grid (in same scale[m/cm] as the voxel grid)
        magnitude: noise multiplier
        """
        device = coords.device

        # Create blur kernels for 3D convolution
        blurx = torch.ones((1, 1, 3, 1, 1), device=device, dtype=torch.float32) / 3
        blury = torch.ones((1, 1, 1, 3, 1), device=device, dtype=torch.float32) / 3
        blurz = torch.ones((1, 1, 1, 1, 3), device=device, dtype=torch.float32) / 3
        ...
        for _ in range(2):
            for dim_idx in range(3):
                # Process each coordinate channel separately
...
class ShufflePoint(object):
    """GPU-accelerated Point Shuffling for point clouds"""
...
class CropBoundary(object):
    """GPU-accelerated Boundary Cropping for point clouds"""
    def __call__(self, data_dict):
...
class ContrastiveViewsGenerator(object):
    """GPU-accelerated Contrastive Views Generator for point clouds"""
    ...
    def __call__(self, data_dict):
        ...
        for key in self.view_keys:
            ...
            if torch.is_tensor(data_dict[key]):
                view1_dict[key] = data_dict[key].clone()
                view2_dict[key] = data_dict[key].clone()
            else:
...
@TRANSFORMS_GPU.register_module()
class MultiViewGenerator(object): # still cpu
    def __init__(
        self,
        global_view_num=2,
...
@TRANSFORMS_GPU.register_module()
class InstanceParser(object): # still cpu
...
class MixUp(object):
    ...
    def __call__(self, batch):
        batch['offset'] = th.cumsum(batch['offset'],dim=0) # new meaning! todo: don't use "offset" in other cases -- too silly
